...
def find_fire_hydrant(image):
    ...
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV) # Convert image to HSV colour space
    mask = cv2.inRange(hsv, np.array([0, 1, 0], dtype='uint8'), np.array([1, 255, 255], dtype='uint8')) # Apply thresholding to obtain binary mask
...
def find_mail_box(image):
    ...
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV) # Convert image to HSV colour space
    mask = cv2.inRange(hsv, np.array([110, 1, 0], dtype='uint8'), np.array([113, 255, 255], dtype='uint8')) # Apply thresholding to obtain binary mask
